...
module Cryal
  ...
  class App < Roda
    route('plan') do |routing| # rubocop:disable Metrics/BlockLength
      routing.on String do |room_id| # rubocop:disable Metrics/BlockLength
        routing.on String do |plan_name| # rubocop:disable Metrics/BlockLength
          ...
          routing.on 'waypoints' do
            routing.is do
              ...
              routing.delete do
                Cryal::PlanService.new(App.config).delete_waypoint(routing, @current_account, room_id)
          ...
          routing.delete do
            Cryal::PlanService.new(App.config).delete_plan(routing, @current_account, room_id, plan_name)
